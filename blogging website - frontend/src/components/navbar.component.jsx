...
const Navbar = () => {
    ...
    const {  userAuth, userAuth: { access_token, profile_img }, setUserAuth, new_notification_available } = useContext(UserContext);

    let { theme, setTheme } = useContext(ThemeContext);

    useEffect(() => {
        if(access_token){
            axios.get('http://localhost:3000/new-notification', {
                headers: {
